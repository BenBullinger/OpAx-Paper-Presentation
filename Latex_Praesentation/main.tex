...
\begin{frame}{Motivation: Beyond Task-Specific Models \hl{ben}}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \vspace{0.5em}\pause
            \begin{block}{Insight}
                Explore to reduce \alert{epistemic uncertainty} about dynamics, not to maximize reward
            \end{block}
        \end{column}
    \end{columns}
    \vspace{0.5em}\pause
    \begin{center}
        \alert{
        Can we efficiently learn accurate dynamics models that generalize to \textit{any} downstream task?
        }
    \end{center}
\end{frame}
...
\]
Goal: Design algorithms for learning $\hat{f}\approx f^\star$ in episodic setting that is good for any reward functions:
% \begin{itemize}
...
\begin{frame}{Main Guarantee}
    \begin{theorem}[Informal]
        ...
        \[
            ...
            \max_{t} \lVert {\color{ETHRed}\sigma_n(s_t,a_t)}\rVert^2_2
            }_{
        ...
        \]
        For linear and RBF kernels, $\displaystyle\lim_{n\to\infty}{\color{ETHRed}\sigma_n(s, a)}\to 0$.
    \end{theorem}\pause
    % \begin{itemize}
...
\begin{corollary}[Informal]
    ...
    then $\hat{\pi}$ is $\varepsilon$-optimal,
    % \[
    %     \max_{\pi} R(\pi) - R(\hat{\pi}) \leq \varepsilon,
    % \]
    where
    \[
    \hat{\pi} \in \argmax_{\pi} {\color{Green}\min_{\eta}{}} R(\pi,\eta),\quad
        s_{t+1} = {\color{Green}\mu_n(s_t,a_t) + \eta(s_n)\beta_n\sigma_n(s_t,a_t) +w_t}.
    \]
\end{corollary}

\begin{center}
\includegraphics[scale=0.2]{Latex_Praesentation/figures/zero-shot.png}
\end{center}
...
\begin{frame}{Experiments: Reduction in Epistemic Uncertainty \hl{ben}}
    \centering
    ...
    \vspace{0.1em}
    \begin{itemize}
        \item OPAX reduces uncertainty faster than random
        \item Active methods outperform random baseline for both GP and PE
        \item Validates theoretical convergence guarantees
        \item Optimism provides slight edge over mean planning
    \end{itemize}
    \begin{center}
        \alert{
        Information-driven exploration works in practice
        }
    \end{center}
\end{frame}
...
\begin{frame}{Experiments: Learning and Zero-Shot Generalization \hl{ben}}
    ...
    \begin{columns}[T] % Align tops
         \begin{column}{0.5\textwidth}
            \textbf{Evaluation} (\fcolorbox{black}{black}{\rule{0pt}{1pt}\rule{1pt}{0pt}}\:: \texttt{H-UCRL}, \fcolorbox{black}{Fuchsia}{\rule{0pt}{1pt}\rule{1pt}{0pt}}\:: \texttt{OpAx}, \fcolorbox{black}{red}{\rule{0pt}{1pt}\rule{1pt}{0pt}}\:: Random)\textbf{:}\vspace{-0.2in}
            \begin{figure}
              \centering
              \subfloat[Trained task\label{fig:a}]{\includegraphics[width=0.45\linewidth]{figures/pendulum-keep-down.png}}
        
              \subfloat[Zero-shot task\label{fig:b}]{\includegraphics[width=0.45\linewidth]{figures/zero-shot-pendulum-swing-up.png}}
            %\caption{Evaluation}
            \label{fig:1}
            \end{figure}
         \end{column}
         ...
         \begin{column}{0.3\textwidth}
            \textbf{Zero-shot swing up task:}\vspace{-0.2in}
            \begin{figure}
              \centering
              \subfloat[\texttt{H-UCRL}\label{fig:a}]{\movie[label=false,height=0.505\linewidth,width=0.505\linewidth]{(play)}{videos/hucrl-zero-shot-swingup-4.mov}}
        
              \subfloat[\texttt{OpAx}\label{fig:b}]{\movie[label=false,height=0.505\linewidth,width=0.505\linewidth]
